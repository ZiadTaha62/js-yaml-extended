...
export class Tokenizer {
  ...
  #handleDirLocal(dir: string): LocalDirParts {
    ...
    if (!alias) throw new WrapperYAMLException(``);
  ...
  #handleDirImport(dir: string): ImportDirParts {
    ...
    if (!alias || !path) throw new WrapperYAMLException(``);
    // remove wrapping escape char if present
    const handledAlias = this.#removeEscChar(alias);
  ...
  #handleExprThis(expr: string): ThisExprParts {
    // remove statring $this and trim, also remove dot if new string starts with a dot
    const data = expr.replace("$this", "").trim().replace(START_WITH_DOT, "");
    ...
    // verify that nodepath is present
    if (!nodepathStr) throw new WrapperYAMLException();
  ...
  #handleExprImport(expr: string): ImportExprParts {
    // remove statring $import and trim, also remove dot if new string starts with a dot
    const data = expr.replace("$import", "").trim().replace(START_WITH_DOT, "");
    // divide expression into parts, first part is <nodepath> and second is [key-value ...]
    const parts = this.#divideExpression(data, 2);
    const nodepathStr = parts[0];
    const keyValueParts = parts.slice(1);
  ...
  #handleExprLocal(expr: string): LocalExprParts {
    ...
    const alias = this.#divideExpression(data, 1);
    if (!alias[0]) throw new WrapperYAMLException();
    const handledAlias = this.#removeEscChar(alias[0]);
  ...
  #divideNodepath(path: string): string[] {
